...
<body class="bg-board text-on-background font-body-lg selection:bg-secondary selection:text-on-secondary overflow-x-hidden">
    ...
    <nav class="flex justify-between items-center w-[98%] mx-auto px-margin py-8 bg-surface shadow-[6px_8px_0px_0px_rgba(0,0,0,0.4)] organic-rotate-1 border-b-4 border-primary/40 sticky z-50 mt-4 wobbly-path scroll-reveal">
        ...
        <div class="hidden md:flex items-center gap-10">
            ...
            @guest
                <a class="font-hand-1 text-xl uppercase tracking-widest px-4 py-2 bg-secondary text-on-secondary rotate-2 hover:-rotate-2 transition-all cursor-pointer shadow-md border-2 border-black" href="{{ route('login') }}">Login</a>
            @endguest
            @auth
                @if(auth()->user()->is_therapist)
                    <a class="font-hand-1 text-xl uppercase tracking-widest text-secondary hover:text-primary transition-all cursor-pointer marker-underline font-bold mr-4" href="{{ route('therapist.dashboard') }}">Therapist Portal</a>
                @endif
                <a class="font-hand-1 text-xl uppercase tracking-widest text-primary hover:text-secondary transition-all cursor-pointer marker-underline font-bold" href="{{ route('dashboard') }}">My Board</a>
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="font-hand-1 text-xl uppercase tracking-widest px-4 py-2 bg-surface-container-highest text-on-surface -rotate-2 hover:rotate-2 transition-all cursor-pointer shadow-md border-2 border-black">Logout</button>
                </form>
            @endauth
        </div>
    ...
    <!-- Footer -->
    <footer class="flex flex-col md:flex-row justify-between items-center px-margin py-8 gap-8 w-[96%] mx-auto bg-surface-container-low text-primary mt-20 border-t border-primary/20 relative z-10 mb-8 shadow-md">
        <div class="font-headline-md uppercase tracking-widest text-2xl text-primary">
            @foreach(str_split(config('app.name', 'THE MIND BOARD')) as $char)
                <span>{!! $char === ' ' ? '&nbsp;' : $char !!}</span>
            @endforeach
        </div>
        <div class="font-body-sm text-sm opacity-70 tracking-wider">&copy; {{ date('Y') }} {{ config('app.name', 'THE MIND BOARD') }} - YOUR MIND MATTERS</div>
        <div class="flex gap-8">
            <a class="font-body-md text-sm text-on-surface-variant hover:text-secondary transition-colors underline" href="#">Privacy</a>
            <a class="font-body-md text-sm text-on-surface-variant hover:text-secondary transition-colors underline" href="#">Terms</a>
            <a class="font-body-md text-sm text-on-surface-variant hover:text-secondary transition-colors underline" href="#">Help & Support</a>
        </div>
